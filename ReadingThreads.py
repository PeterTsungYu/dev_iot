...
RS485_port = serial.Serial(
    port='/dev/ttyUSB0', # input('RS485 port: ') 
    baudrate=19200, 
    bytesize=8, 
    stopbits=1, 
    parity='N'
...
RS232_port = serial.Serial(
    port='/dev/ttyUSB1', # input('RS232 port: ') 
    baudrate=9600, 
    bytesize=8, 
    stopbits=1, 
    parity='N'
...
Scale_port = serial.Serial(
    port='/dev/ttyUSB2', # input('Scale port: ') 
    baudrate=9600, 
    bytesize=8, 
    stopbits=1, 
    parity='N'
...
Adam_data_analyze = threading.Thread(
    ...
    args=(kbinterrupt_event, ticker, sample_time, ADAM_TC_slave,),
    )
...
GA_data_analyze = threading.Thread(
    ...
    args=(kbinterrupt_event, ticker, sample_time, GA_slave,),
    )
MFC_data_analyze = threading.Thread(
    target=Modbus.MFC_data_analyze, 
    args=(kbinterrupt_event, ticker, sample_time, MFC_slave,),
    )
...
Scale_data_analyze = threading.Thread(
    ...
    args=(kbinterrupt_event, ticker, sample_time, Scale_slave,),
    )
...
DFM_data_analyze = threading.Thread(
    ...
    args=(kbinterrupt_event, ticker, sample_time, DFM_slave,),
    )
