...
class ADS1256:
    def __init__(self):
        self.rst_pin = ADDA_config.RST_PIN
        self.cs_pin = ADDA_config.CS_PIN
        self.drdy_pin = ADDA_config.DRDY_PIN

    # Hardware reset
    def ADS1256_reset(self):
        ADDA_config.digital_write(self.rst_pin, GPIO.HIGH)
        ADDA_config.delay_ms(200)
        ADDA_config.digital_write(self.rst_pin, GPIO.LOW)
        ADDA_config.delay_ms(200)
        ADDA_config.digital_write(self.rst_pin, GPIO.HIGH)
    
    def ADS1256_WriteCmd(self, reg):
        ADDA_config.digital_write(self.cs_pin, GPIO.LOW)#cs  0
        ADDA_config.spi_writebyte([reg])
        ADDA_config.digital_write(self.cs_pin, GPIO.HIGH)#cs 1
    
    def ADS1256_WriteReg(self, reg, data):
        ADDA_config.digital_write(self.cs_pin, GPIO.LOW)#cs  0
        ADDA_config.spi_writebyte([CMD['CMD_WREG'] | reg, 0x00, data])
        ADDA_config.digital_write(self.cs_pin, GPIO.HIGH)#cs 1
        
    def ADS1256_Read_data(self, reg):
        ADDA_config.digital_write(self.cs_pin, GPIO.LOW)#cs  0
        ADDA_config.spi_writebyte([CMD['CMD_RREG'] | reg, 0x00])
        data = ADDA_config.spi_readbytes(1)
        ADDA_config.digital_write(self.cs_pin, GPIO.HIGH)#cs 1
    ...
    def ADS1256_WaitDRDY(self):
        for i in range(0,400000,1):
            if(ADDA_config.digital_read(self.drdy_pin) == 0):
    ...
    #The ADDA_configuration parameters of ADC, gain and data rate
    def ADS1256_ADDA_configADC(self, gain, drate):
        self.ADS1256_WaitDRDY()
        buf = [0,0,0,0,0,0,0,0]
        buf[0] = (0<<3) | (1<<2) | (0<<1)
        buf[1] = 0x08
        ...
        ADDA_config.digital_write(self.cs_pin, GPIO.LOW)#cs  0
        ADDA_config.spi_writebyte([CMD['CMD_WREG'] | 0, 0x03])
        ADDA_config.spi_writebyte(buf)
        
        ADDA_config.digital_write(self.cs_pin, GPIO.HIGH)#cs 1
        ADDA_config.delay_ms(1) 
    ...
    def ADS1256_init(self):
        ...
        if id == 3 :
            ...
            return -1
        self.ADS1256_ADDA_configADC(ADS1256_GAIN_E['ADS1256_GAIN_1'], ADS1256_DRATE_E['ADS1256_30000SPS'])
        return 0
        
    def ADS1256_Read_ADC_Data(self):
        self.ADS1256_WaitDRDY()
        ADDA_config.digital_write(self.cs_pin, GPIO.LOW)#cs  0
        ADDA_config.spi_writebyte([CMD['CMD_RDATA']])
        # ADDA_config.delay_ms(10)

        buf = ADDA_config.spi_readbytes(3)
        ADDA_config.digital_write(self.cs_pin, GPIO.HIGH)#cs 1
